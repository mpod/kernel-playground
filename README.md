...
```
pi@raspberrypi:~/lkm $ ls /sys/bus/iio/devices
iio:device0  iio_sysfs_trigger
```

To create a trigger execute following command.
...
```
root@raspberrypi:~# ls /sys/bus/iio/devices
iio:device0  iio_sysfs_trigger  trigger0
```

To connect trigger with the BMP280 driver write trigger's name to 
...
Ispect data with `hexdump` program.
...
First 2 bytes are temperature measurements, second 2 bytes are pressure 
